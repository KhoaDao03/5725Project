...
ApplyXlogUntil(StringInfo input_message) {
    ...
    /*
     * Adaptive Smart Replay (ASR) integration:
     * Limit the number of records replayed per call based on current budget.
     * This prevents excessive CPU usage during light load and ensures responsive
     * degradation under heavy write load.
     */
    int replay_budget = ASR_GetCurrentBudget();	/* Dynamic budget from controller */
...
//        }
        ...
        /*
         * Adaptive Smart Replay: track replayed records and respect budget.
         * records_replayed counts logical replay units (typically records processed).
         * When budget exhausted, exit and return; caller will invoke again.
         */
        records_replayed++;
        ASR_RecordReplayTask(1);  /* Record one task completed */
        
        if (records_replayed >= replay_budget)
        {
            /* Budget exhausted; return and let caller invoke again */
            break;
        }
    }
#ifdef ENABLE_DEBUG_INFO
